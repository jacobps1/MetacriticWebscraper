...
from webscrape.searchresults.categorydetails.albumdetail.albumparse.album_review import album_review

class album_user_parser(object):
    '''
    Constructor for Album_User_Parser object
    Parameters:
        link represents the link (url) of the album's user reviews
    '''
    ...
    def _get_reviews(self):
        ...
        start_index = self.html.find("<ol class=\"reviews user_reviews\">") + len("<ol class=\"reviews user_reviews\">")
        ...
        start_index = html.find("class=\"review user_review first_review\">") 
        ...
        start_index = start_index + len("class=\"review user_review first_review\">")
        
        current_review = html[start_index:]
        
        #create the album_review object that will parse the review
        review = album_review(current_review) 
        ...
        start_index = html.find("class=\"review user_review")
        
        #while there are still reviews left
        while (start_index != -1):
            
            #get the first review
            start_index = start_index + len("class=\"review user_review")
